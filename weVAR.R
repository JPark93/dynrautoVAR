...
#'   if `use.mi = TRUE`.
#' @param MLVAR if `MLVAR = TRUE`, time-series for multiple subjects will be chained and a group-level model will be fitted to the entire group.
#' @param avgnet if `avgnet = TRUE`, parameter estimates will be averaged across the sample and distributional characteristics will be returned.
...
weVAR <- function(dataframe,
  ...
  for (k in unique(dataframe[, id])) {
    ...
    rawdata <- dynr.data(pseudo.dat, id = id, time = time, observed = varnames)
    # Model Fitting
    model <- dynr.model(
    ...
    if (use.mi == TRUE) {
      ...
      results <- dynr.cook(model, verbose = verbose)
    ...
    group.info = NULL
    if(MLVAR == TRUE){
      message('Fitting an ML VAR to the Chained Time-Series!')
      ...
      # 2. Fit the Fuckin' Model
        rawdata <- dynr.data(chain.temp, id = id, time = time, observed = varnames)
            model <- dynr.model(
              dynamics = dynm, measurement = meas,
              noise = mdcov, initial = initial, data = rawdata,
              outfile = paste0(dir, "GroupMod", ".c")
            )
            model$ub[1:nv^2] <- 1.00
            model$lb[1:nv^2] <- -1.00
            # Cooking the dynr model
